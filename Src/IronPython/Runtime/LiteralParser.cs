...
namespace IronPython.Runtime {
    public static class LiteralParser {
        ...
        private static string DoParseString<T>(T[] data, int start, int length, bool isRaw, bool isUniEscape, bool normalizeLineEndings) where T : unmanaged {
        ...
        private static unsafe char ToChar<T>(T x) where T : unmanaged {
            switch (sizeof(T)) {
                case 1: return (char)*((byte*)&x);
                case 2: return *((char*)&x);
        ...
        private static bool TryParseInt<T>(T[] text, int start, int length, int b, out int value, out int consumed) where T : unmanaged {
            value = 0;
