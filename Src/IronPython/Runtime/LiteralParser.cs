...
namespace IronPython.Runtime {
    public static class LiteralParser {
        public static string ParseString(char[] text, int start, int length, bool isRaw, bool isUniEscape, bool normalizeLineEndings) {
            ...
            string result = DoParseString(text, start, length, isRaw, isUniEscape, normalizeLineEndings, ch => ch);
        ...
        public static string ParseString(byte[] bytes, int start, int length, bool isRaw) {
            ...
            string result = DoParseString(bytes, start, length, isRaw, isUniEscape: true, normalizeLineEndings: false, b => (char)b);

            return result ?? bytes.MakeString(start, length);
        }

        private delegate char ToChar<T>(T ch);

        private static string DoParseString<T>(T[] data, int start, int length, bool isRaw, bool isUniEscape, bool normalizeLineEndings, ToChar<T> toChar) {
            StringBuilder buf = null;
            int i = start;
            int l = start + length;
            int val;
            while (i < l) {
                char ch = toChar(data[i++]);
                if ((!isRaw || isUniEscape) && ch == '\\') {
                    ...
                    if (ch == 'u' || ch == 'U') {
                        ...
                        int max = 16;
                        if (isUniEscape) {
                            if (TryParseInt(data, i, len, max, toChar, out val, out int consumed)) {
                        ...
                        switch (ch) {
                            ...
                            case '\'': buf.Append('\''); continue;
                            case '\"': buf.Append('\"'); continue;
                            case '\r': if (i < l && toChar(data[i]) == '\n') i++; continue;
                            case '\n': continue;
                            case 'N': {
                                    IronPython.Modules.unicodedata.PerformModuleReload(null, null);
                                    if (i < l && toChar(data[i]) == '{') {
                                        i++;
                                        StringBuilder namebuf = new StringBuilder();
                                        bool namecomplete = false;
                                        while (i < l) {
                                            char namech = toChar(data[i++]);
                                            if (namech != '}') {
                                ...
                                continue;
                            case 'x': //hex
                                if (!TryParseInt(data, i, 2, 16, toChar, out val, out int _)) {
                                    goto default;
                                }
                                buf.Append((char)val);
                                i += 2;
                            ...
                            case '7': {
                                    int onechar;
                                    val = ch - '0';
                                    if (i < l && HexValue(toChar(data[i]), out onechar) && onechar < 8) {
                                        val = val * 8 + onechar;
                                        i++;
                                        if (i < l && HexValue(toChar(data[i]), out onechar) && onechar < 8) {
                                            val = val * 8 + onechar;
                    ...
                    if (i < data.Length && toChar(data[i]) == '\n') {
                        i++;
                    }
                    buf.Append('\n');
                } else {
            ...
            while (i < l) {
                ...
                if (!isRaw && ch == '\\') {
                    ...
                    switch (ch) {
                        ...
                        case '\n': continue;
                        case 'x': //hex
                            if (!TryParseInt(text, i, 2, 16, ch => ch, out val, out int _)) {
                                goto default;
                            }
                            buf.Add((byte)val);
                            i += 2;
        ...
        private static bool TryParseInt<T>(T[] text, int start, int length, int b, ToChar<T> toChar, out int value, out int consumed) {
            value = 0;
            ...
            for (int i = start, end = start + length; i < end; i++) {
                if (HexValue(toChar(text[i]), out int onechar) && onechar < b) {
